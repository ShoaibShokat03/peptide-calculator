class PeptideCalculator {
  constructor(target = "#peptide-calculator") {
    ...
    this.isFormulateRendered = false;
  ...
  injectStyle() {
    ...
    padding:20px;
            ...
            .peptide-calculator {
                max-width: 420px; width: auto; padding: 20px;
                background: linear-gradient(145deg, #2b3a90 0%, #0d1e70 100%);
                border: 1px solid #071149; border-radius: 12px;
            ...
            .input-group select, .input-group input {
                padding: 10px; background: #071149; border: 1px solid #3a4a9a;
                color: white; font-size: 1.1rem; border-radius: 8px; text-align: center;
            }
  ...
  _mainForm() {
    ...
    waterInput.oninput = () => {
      this.formData.waterValue = parseFloat(waterInput.value) || 0;
      this._render();
    };
    ...
    waterUnitSelect.onchange = () => {
      this.formData.waterUnit = waterUnitSelect.value;
      this._render();
    };
  ...
  _renderPeptides() {
    ...
    this.formData.peptides.forEach((peptide, index) => {
      ...
      peptideInput.oninput = () => {
        this._updatePeptide(peptide.id, 'peptideQuantity', parseFloat(peptideInput.value) || 0);
      };
      ...
      doseInput.oninput = () => {
        this._updatePeptide(peptide.id, 'peptideDosageValue', parseFloat(doseInput.value) || 0);
      };
      ...
      doseUnitSelect.onchange = () => {
        ...
        // Convert value when unit changes to maintain same absolute quantity
        ...
        doseInput.value = newValue; // Update the visible input field
      };
  ...
  // --- CALCULATION & RESULTS ---
  _formulate(formData) {
    // --- 1. Get water volume in milliliters (mL) ---
    let waterVolumeInML = formData.waterValue;
    if (formData.waterUnit === "iu") {
      waterVolumeInML = formData.waterValue / 100;
    }
    ...
    formData.peptides.forEach(peptide => {
      // Add peptide quantity (already in mg)
      totalPeptideInMg += peptide.peptideQuantity;

      // Add desired dose, converting from mcg if necessary
      let peptidePerDoseInMg = peptide.peptideDosageValue;
      if (peptide.peptideDosageUnit === "mcg") {
        peptidePerDoseInMg = peptide.peptideDosageValue / 1000;
      }
    ...
    if (concentrationInMgPerML > 0 && totalDesiredDoseInMg > 0) {
      // Calculate how many doses we can get from the total peptide
    ...
    const formattedUnits = unitsToDraw.toFixed(0);
    const formattedConcentration = concentrationInMgPerML.toFixed(2);
    const formattedTotalDoses = isFinite(totalDosesInVial) ? totalDosesInVial.toFixed(0) : '0';
    const formattedVolumePerDose = volumePerDoseInML.toFixed(2);

    // --- 5. Create dose summary for display ---
    let doseDescription = '';
    if (formData.peptides.length === 1) {
      const peptide = formData.peptides[0];
      doseDescription = `${peptide.peptideDosageValue}${peptide.peptideDosageUnit}`;
    } else {
      const doses = formData.peptides.map(peptide =>
        `${peptide.peptideDosageValue}${peptide.peptideDosageUnit}`
      ).join(' + ');
      doseDescription = doses;
    }
    ...
    const barFillPercent = (unitsToDraw / formData.scaleUnits) * 100;
    scale.innerHTML = `
    ...
    let scaleDivide = 6;
    let valueGap = 5;
    if (formData.scaleUnits == 50) { scaleDivide = 10; valueGap = 5; }
    else if (formData.scaleUnits == 100) { scaleDivide = 10; valueGap = 10; }
