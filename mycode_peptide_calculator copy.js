class PeptideCalculator {
  ...
  _mainForm() {
    ...
    waterInput.addEventListener('input', (e) => {
      this.formData.waterValue = parseInt(e.target.value);
      console.log(e.target.value);
      this._render();
    });
    const waterUnitSelect = this._elem("select", "unit-select");
    waterInputWithUnit.appendChild(waterUnitSelect);
    const waterOptions = ["ml", "iu"];
    waterOptions.forEach(option => {
      const optionElement = this._elem("option", null, option);
      if (option === this.formData.waterUnitSelect) {
        optionElement.selected = true;
      }
      waterUnitSelect.appendChild(optionElement);
    });
  ...
  _formulate(formData) {
    ...
    if (this.formData.waterUnit === "iu") {
      this.formData.waterValue = this.formData.waterValue * 1000;
    }

    let FormulateScale = formData.scaleUnits;

    const totalPeptides = formData.peptidesQuantitie;
    const totalWater = formData.waterValue;

    // Concentration
    const concentration = totalPeptides / totalWater;

    // Dose per vial
    const dosePerVial = formData.peptidesDosageValue;
    const Dose = totalPeptides * 1000 / dosePerVial;

    // Volume per dose
    const volumePerDose = (dosePerVial / 1000) / 1;

    // Units to Draw
    const unitsToDraw = volumePerDose * 100;

    const FormulateValue = unitsToDraw.toFixed(2);
    console.log(unitsToDraw);

    let dosageValueShow = 0;
    if (this.formData.peptidesDosageUnit === "mg") {
      dosageValueShow = this.formData.peptidesDosageValue / 1000;
    } else {
      dosageValueShow = this.formData.peptidesDosageValue;
    }
    ...
    if (FormulateScale == 30) {
      scaleDivide = 6; valueGap = 5;
    } else if (FormulateScale == 50) {
      scaleDivide = 10; valueGap = 5;
    } else if (FormulateScale == 100) {
      scaleDivide = 10; valueGap = 10;
    }
    ...
    const list = [
      `Draw ${FormulateValue} units for ${dosageValueShow} ${this.formData.peptidesDosageUnit} doses`,
    ]
  ...
  _show() {
    let box = document.querySelector(".show-box");
    if (!box) {
      box = this._elem("div", "show-box");
      this.target.appendChild(box);
    }
    let row = `<div style="color:white;margin-top:10px;">`;
    Object.entries(this.formData).forEach(([key, value]) => {
      row += `<div><strong>${key}:</strong> ${value}</div>`;
    });
    row += `</div>`;
    box.innerHTML = row;
  }
